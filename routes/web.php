...
use App\Http\Controllers\Admin\PaymentReportController; // ✅ Report controller
use App\Models\User;
use App\Models\Car;
...
Route::prefix('admin')->middleware('admin')->group(function () {
    ...
    // ✅ Reports routes
    Route::get('/reports/payments', [PaymentReportController::class, 'index'])->name('admin.reports.payments');
    Route::get('/reports/payments/export', [PaymentReportController::class, 'exportCsv'])->name('admin.reports.payments.export');
});
...
// ------------------- checkout for a specific reservation ---------------- //
Route::middleware(['auth','restrictAdminAccess'])->group(function () {
    Route::get('/checkout/reservation/{reservation}', [PaymentController::class, 'createForReservation'])->name('checkout.reservation');
    Route::post('/checkout/reservation/{reservation}', [PaymentController::class, 'storeForReservation'])->name('checkout.reservation.process');
});

// ------------------- generic checkout (optional) ------------------------ //
Route::get('/checkout',        [PaymentController::class, 'create'])->name('checkout');
Route::post('/checkout',       [PaymentController::class, 'store'])->name('checkout.process');

Route::get('/payment/success', [PaymentController::class, 'success'])->name('payment.success');
Route::get('/payment/cancel',  [PaymentController::class, 'cancel'])->name('payment.cancel');

// ------------------- invoice download ---------------------------------- //
Route::get('/payment/{payment}/invoice', [PaymentController::class, 'downloadInvoice'])
